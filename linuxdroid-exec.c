...
static const char* linuxdroid_rewrite_executable(const char* filename, char* buffer, int buffer_len)
{
	strcpy(buffer, "/data/data/com.linuxdroid/files/usr/bin/");
...
int execve(const char* filename, char* const* argv, char *const envp[])
{
	...
	filename = linuxdroid_rewrite_executable(filename, filename_buffer, sizeof(filename_buffer));
	...
	*newline_location = 0;
	...
	char interp_buf[512];
	const char* new_interpreter = linuxdroid_rewrite_executable(interpreter, interp_buf, sizeof(interp_buf));
